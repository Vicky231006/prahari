...
class Alert(Base):
    ...
    raw_events = Column(JSON, nullable=True, default=list)
...
class IdentityProfile(Base):
    ...
    customer_type = Column(String(32))
    customer_segment = Column(String(32))
    kyc_status = Column(String(32))
    account_age_days = Column(Integer, default=0)
    customer_since = Column(String(32))
    primary_branch = Column(String(128))
    region = Column(String(64))
    risk_tier = Column(String(32))
    current_balance = Column(Float, default=0.0)
    average_daily_volume = Column(Float, default=0.0)
    monthly_txn_count = Column(Integer, default=0)
    dormant_account_flag = Column(Boolean, default=False)
    vip_flag = Column(Boolean, default=False)
    previous_alerts_count = Column(Integer, default=0)
    previous_cases_count = Column(Integer, default=0)
    fraud_history_count = Column(Integer, default=0)
    typical_login_hours = Column(JSON, default=list)
    typical_countries = Column(JSON, default=list)
    typical_channels = Column(JSON, default=list)
    preferred_payment_method = Column(String(64))
    device_trust_score = Column(Float, default=0.0)
